...
#include "OGL.h"
#include "MaterialUsingHeaders.h";    //Texture Header File
...
GLuint TEXTURE_MARBLE = 0;         //Texture variable
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("3D_Light_Cube");
	int iRet = 0;
	bool bDone = false;
...
void display(void)
{
	...
	for (int i = 0; i < sizeof(face_indicies) / sizeof(face_indicies[0]); i++)
	{
		for (int j = 0; j < 3; j++)
		{
			int vi = face_indicies[i][j];                //for vertex
			int ni = face_indicies[i][j + 3];			 //for normal
			int ti = face_indicies[i][j + 6];			 //for textures
...
void Update()
{
	angle_teapot = angle_teapot + 0.1f;
