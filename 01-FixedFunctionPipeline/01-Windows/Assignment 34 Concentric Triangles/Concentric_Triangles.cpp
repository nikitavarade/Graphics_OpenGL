...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("Concentric Triangles");
	int iRet = 0;
	bool bDone = false;
...
void display(void)
{
	...
	for (count = 0.0f; count <= 1.0f; count=count+0.1f)
	{
		...
		glVertex2f(0.0f, 1.0f-(count));
		glVertex2f(-1.0f + (count), -1.0f + (count));
		glVertex2f(1.0f - (count), -1.0f + (count));
		glEnd();
	...
	{
		glBegin(GL_LINE_LOOP);
		glVertex3f(0.0f, 1.0f * (i*0.1f),0.0f);
		glVertex3f(-1.0f * (i*0.1f), -1.0f * (i*0.1f),0.0f);
		glVertex3f(1.0f * (i*0.1f), -1.0f * (i*0.1f),0.0f);
		glEnd();
		//use SwapBuffer(ghdc) for double buffering instead of glFush() 
	}*/
