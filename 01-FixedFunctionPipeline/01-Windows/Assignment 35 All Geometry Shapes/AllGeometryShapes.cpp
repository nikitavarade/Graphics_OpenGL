...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("AllgeometryShapes");
	int iRet = 0;
	bool bDone = false;
...
void Circle(GLfloat Radius, GLfloat x, GLfloat y)
{
	GLfloat angle = 0.0f;
	glBegin(GL_POINTS);

	for (angle = 0.0f; angle < 2.0 * M_PI; angle = angle + 0.000001)
	{
		glVertex3f((GLfloat)cos(angle)*Radius+x, (GLfloat)sin(angle)*Radius+y, 0.0f);
...
void InCircle()
{
	...
	GLfloat angle = 0.0f, y1 = 1.0f, y2 = -1.0f, y3 = -1.0f, Ox = 0.0f, Oy = 0.0f, Perimeter = 0.0f, S = 0.0f, z = 0.0f;
	GLfloat ax, bx, cx, ay, by, cy;
	...
	Ox = ((x1 * b) + (x2 * c) + (x3 * a)) / Perimeter;
	Oy = ((y1 * b) + (y2 * c) + (y3 * a)) / Perimeter;
	...
	for (angle = 0.0f; angle < 2.0 * M_PI; angle = angle + 0.000001)
	{
		glVertex3f((GLfloat)cos(angle)* Radius + Ox-0.03, (GLfloat)sin(angle)* Radius + Oy-0.065 , 0.0f);
