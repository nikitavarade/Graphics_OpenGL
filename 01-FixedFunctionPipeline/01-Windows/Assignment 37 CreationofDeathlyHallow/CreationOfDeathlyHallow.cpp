...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("Creation of Deathly Hallows");
	int iRet = 0;
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Creation of Deathly Hallows "),
...
void Circle(void)
{
	GLfloat count = 0.0f, x1 = -0.3f, x2 = 0.3f, x3 = 0.0f, a = 0.0f, b = 0.0f, c = 0.0f, Radius = 0.0f, Area = 0.0f;
	GLfloat  angle = 0.0f, y1 = -0.3f, y2 = -0.3f, y3 = 0.3f, Ox = 0.0f, Oy = 0.0f, Perimeter = 0.0f, S = 0.0f, z = 0.0f;
	...
	c = sqrtf(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1)));

	Perimeter = a + b + c;

	S = Perimeter / 2.0f; //Semiperimeter

	//Origin of Circles(Center)
	Ox = ((a * x1) + (b * x2) + (c * x3)) / Perimeter;

	Oy = ((a * y1) + (b * y2) + (c * y3)) / Perimeter;

	Radius = sqrtf((S * (S - a) * (S - b) * (S - c))) / S;
...
void Triangle(void)
{
	...
	GLfloat count = 0.0f, x1 = -0.3f, x2 = 0.3f, x3 = 0.0f, y1 = -0.3f, y2 = -0.3f, y3 = 0.3f, z = 0.0f;
	glLineWidth(3.0f);

	glBegin(GL_LINES);
	glVertex3f(0.0f, y3, z);
	glVertex3f(x1, y1, z);
	glVertex3f(x1, y1, z);
	glVertex3f(x2, y2, z);
	glVertex3f(x2, y2, z);
...
void display(void)
{
	...
	static GLfloat angle_r = 0.0f, Tx = 1.0f, Ty = -1.0f;
	static GLfloat angle_t = 0.0f, Tx1 = -1.0f, Ty1 = -1.0f;
	static GLfloat angle_l = 0.0f, Tx2 = 0.0f, Ty2 = 1.0f;
	...
	glTranslatef(Tx, Ty, -2.0f);
	glRotatef(angle_r, 1.0f, 0.0f, 0.0f);
	Circle();


	glLoadIdentity();
	glTranslatef(Tx1, Ty1, -2.0f);
	glRotatef(angle_t, 0.0f, 1.0f, 0.0f);
	Triangle();

	glLoadIdentity();
	glTranslatef(Tx2, Ty2, -2.0f);
	glRotatef(angle_l, 0.0f, 0.0f, 1.0f);
	Line();
	//for triangle
	if (Tx1 <= 0 && Ty1 <= 0)
	{
		Tx1 = Tx1 + 0.001;
		Ty1 = Ty1 + 0.001;
		angle_t = angle_t + 0.51f;
		if (angle_t > 360.0f)
		{
			angle_t = 0.0f;
		}
	}
	else if (angle_t <= 360.0f)
	{
	...
	else if (angle_r <= 360.0f)
	{

		angle_r = angle_r + 0.3f;
	}

	//for line
	if (Ty2 > 0.0f)
	{
		angle_l = angle_l + 0.21f;
		if (angle_l >= 360.0f)
		{
			angle_l = 0.0f;
		}
	...
	else if (angle_l < 360.0f)
	{

		angle_l = angle_l + 0.1f;
	}
