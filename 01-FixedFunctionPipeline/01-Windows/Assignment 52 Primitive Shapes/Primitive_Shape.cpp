...
GLfloat PointDifference = 2.0f / 3.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("PrimitiveShapes");
	int iRet = 0;
	bool bDone = false;
...
void Shape1(void)
{
	...
	for (int i = 1; i <= 4; i++)
	{
		for (int j = 1; j <= 4; j++)
		{
			glVertex3f(init_x, init_y, 0.0f);
			init_x = init_x + PointDifference;
		}
		init_x = -1.0f;
		init_y = init_y - PointDifference;
	}
...
void Shape2(void)
{
	...
	for (int i = 0; i < 3; i++)
	{
		glVertex3f(-1.0f, 1.0f - i * PointDifference, 0.0f);//Horizontal Line
		glVertex3f( 1.0f, 1.0f - i * PointDifference, 0.0f);

		glVertex3f(-1.0f + i * PointDifference,  1.0f, 0.0f);//Vertical Lines
		glVertex3f(-1.0f + i * PointDifference, -1.0f, 0.0f);

		glVertex3f(-1.0f + (i+1) * PointDifference , 1.0f, 0.0f);//Diagonal Lines left top to center
		glVertex3f(-1.0f						   , 1.0f - (i+1) * PointDifference, 0.0f);
		
		glVertex3f(1.0f, -1.0f + i * PointDifference, 0.0f);//Biagonal Lines right bottom to center
		glVertex3f(1.0f - i * PointDifference, -1.0f, 0.0f);
	}
...
void Shape3(void)
{
	...
	for (int i = 0; i < 4; i++)
	{
		glVertex3f(1.0f, 1.0f - i * PointDifference, 0.0f);//Horizontal Lines
		glVertex3f(-1.0f, 1.0f - i * PointDifference, 0.0f);
...
void Shape6(void)
{
	...
	glVertex3f(-1.0f + PointDifference, 1.0f, 0.0f);
	glVertex3f(-1.0f, 1.0f, 0.0f);
	glVertex3f(-1.0f, -1.0f, 0.0f);
	glVertex3f(-1.0f + PointDifference, -1.0f, 0.0f);
