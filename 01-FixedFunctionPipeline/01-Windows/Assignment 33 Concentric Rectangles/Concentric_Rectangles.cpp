...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("Concentric Circle");
	int iRet = 0;
	bool bDone = false;
...
void Concentric(void)
{
	...
	{
	angle = (GLfloat)((2.0f * 3.1428) * i) / 1000;
	glVertex3f((GLfloat)cos(angle)*count, (GLfloat)sin(angle)*count, 0.0f);
	}


	glEnd*/
	GLfloat PI = 3.14f;
	for (count = 0.0f; count <= 2.0f; count = count + 0.25f)
	{
		glPointSize(8.0f);
		glBegin(GL_POINTS);
		if (count == 0.0f)
...
void display(void)
{
	...
	GLfloat count = 0.0f, x=1.0f, y= 1.0f, z = 1.0f;

	//glColor3f(1.0f, 0.0f, 0.0f);
	glBegin(GL_LINES);
	for (count = 0.0f; count < 1.0f; count = count + 0.1f)
	{
		
		glVertex3f(-1.0f + count, 1.0f - count, z);
		glVertex3f(-1.0f + count, -1.0f + count, z);
