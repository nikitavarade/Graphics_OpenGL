...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("NativeTrianglePerspective");
	int iRet = 0;
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("NativeTrianglePerspective"),
...
{
	//slices/sectorcount/longitude are same
	//stacks/latitude are same
	
	GLfloat x1, y1, z1, x2, y2, z2, xy1, xy2;	//vertex position
	GLfloat nx1, ny1, nz1, nx2, ny2, nz2, length_inverse = 1.0f / radius; //vertex normal

	GLfloat SectorSteps = (2 * PI) / slices;
	GLfloat StackSteps = PI / stacks;

	GLfloat stackangle1, stackangle2;
	GLfloat sectorangle1, sectorangle2;

	for (int j = 0; j < stacks; j++)
	{
		stackangle1 = ((PI / 2) - (j * StackSteps));
		stackangle2 = ((PI / 2) - ((j + 1) * StackSteps));

		xy1 = radius * (GLfloat)cos(stackangle1);
		z1= radius * (GLfloat)sin(stackangle1);

		xy2 = radius * (GLfloat)cos(stackangle2);
		z2 = radius * (GLfloat)sin(stackangle2);

		glBegin(GL_LINE_LOOP);
		for (int i = 0; i < slices; i++)
		{
			sectorangle1 = i * SectorSteps;
			x1= xy1 * (GLfloat)cos(stackangle1);
			y1= xy1* (GLfloat)sin(stackangle1);
			sectorangle2 = i * SectorSteps;
			x2= xy2 * (GLfloat)cos(stackangle2);
			y2= xy2 * (GLfloat)sin(stackangle2);

			//Normal Calculations
			nx1 = x1 * length_inverse;
			ny1 = y1 * length_inverse;
			nz1 = z1 * length_inverse;

			nx2 = x2 * length_inverse;
			ny2 = y2 * length_inverse;
			nz2 = z2 * length_inverse;
