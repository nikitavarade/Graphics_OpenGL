...
GLuint vbo;
GLuint mvpUniform;
mat4 orthgraphicProjectionMatrix;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szAppName[] = TEXT("Orthographic_Triangle");
	int iRet = 0;
	bool bDone = false;
...
int initialize()
{
	...
	orthgraphicProjectionMatrix = mat4::identity();
...
void display(void)
{
	...
	modelViewProjectionMatrix = orthgraphicProjectionMatrix * modelViewMatrix;
...
void ReSize(int width, int height)
{
	...
	if (width < height)
	{
		orthgraphicProjectionMatrix = ortho(-100.0f, 100.0f, -100.0f * (height / width), 100.0f * (height / width), -100.0f, 100.0f);
	}
	else
	{
		orthgraphicProjectionMatrix = ortho(-100.0f * (width / height), 100.0f * (width / height), -100.0f, 100.0f, -100.0f, 100.0f);
